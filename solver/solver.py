...
def LetterErrorRate(pred_y, true_y):
    ...
    #print("pred_y :",pred_y)
    #print("true_y :", true_y)
    #print("ed_accmulate",ed_accumalate)
...
def tensor2text(y, vocab):
    ...
    res=[]
    for rounded in rounded_y:
        rounded_tmp = np.array([vocab[str(n)] for n in rounded])
        res.append(rounded_tmp)
    #rounded_y = torch.tensor([vocab[str(n)] for n in rounded_y])
    return res
...
def batch_iterator(
    ...
    if label_smoothing == 0.0 or not (is_training):
        ...
        # --------------------------------* for test *-------------------------------------
        #print("pred :",tensor2text(torch.max(pred_y.permute(0, 2, 1), dim=2)[1].cpu().numpy(), vocab_dict))
        #print("true :",tensor2text(true_y.cpu().data.numpy(), vocab_dict))
        # --------------------------------* -------------------------------------
