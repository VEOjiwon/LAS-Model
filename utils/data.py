...
def OneHotEncode(idx, max_idx=1500):
    # 한글 .. max_idx = 1500
    idx = int(idx)
...
class AudioDataset(Dataset):
    def __init__(self, params, name="train"):
        ...
        with open(params["data"][name], "rt", encoding="utf8") as t:
    ...
    def __getitem__(self, index):
        ...
        #print("utt id : ", utt_id)
        #print("path : ", path)
        #print("max idx : ", ([int(i) for i in targets.strip().split(" ")]))
        #print("max : ", max([int(i) for i in targets.strip().split(" ")]))
        #print("-"*50)
...
def collate_fn(batch):
    ...
    for _, feat, feat_len, target, target_len in batch:
        padded_features.append(np.pad(feat, ((0, max_feature_length - feat_len), (0, 0)), mode="constant", constant_values=0.0,))
        #tar_padds = [OneHotEncode(PAD, 30) for u in range((max_target_length - len(target)))] # for english
        tar_padds = [OneHotEncode(PAD, 1500) for u in range((max_target_length - len(target)))] # for korea
        # print(len(target))
        # print(len(target[1]))
        ...
        # )
        i += 1
    features = torch.FloatTensor(np.array(padded_features))
    features_length = torch.IntTensor(features_length)
